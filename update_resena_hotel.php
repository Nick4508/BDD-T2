...
<?php 
    ...
    $id_producto = $_GET['id'];
    $query = mysqli_query($conexion,"SELECT opinion FROM resena_hotel WHERE id_usuario = $id_usuario and id_hotel = $id_producto");
    $row = mysqli_fetch_assoc($query)['opinion'];
    echo 'Opinion anterior : '.$row.'<br>'; 
?>
<form action="send_resena_hotel.php" method="post">
